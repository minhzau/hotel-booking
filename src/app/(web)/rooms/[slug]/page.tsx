...
import { use, useState } from "react"; // Import `use` from React 18
...
const RoomDetails = (props: { params: Promise<{ slug: string }> }) => {
  const params = use(props.params); // Unwrap the `params` promise here
  const { slug } = params; // Now destructure `slug` after unwrapping
